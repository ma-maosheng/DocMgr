...
namespace DocMgr.ViewModels.YearlyArchive
{
    internal static class ArchiveReturnAbnormalReportPrintDocumentFactory
    {
        ...
        /// <summary>资料摘要/侧栏明细按正文估行后的上限。</summary>
        private const int SideDetailMaxLines = 3;

        /// <summary>灭失资料明细按正文估行后的上限（撑满下限）。</summary>
        ...
        internal static FlowDocument Create(ArchiveReturnAbnormalReportPrintData data)
        {
            ...
            string borrowText = BuildItemText(data.BorrowItemLines);
            string intactText = BuildItemText(data.IntactReturnItemLines);
            string lossText = BuildItemText(data.LossItemLines);

            double summaryRowHeight = PrintPageLayoutSupport.ResolveSpannedContentRowHeightDip(
                summaryText,
                SideDetailMaxLines);
            double borrowRowHeight = PrintPageLayoutSupport.ResolveSpannedContentRowHeightDip(
                borrowText,
                SideDetailMaxLines);
            double intactRowHeight = PrintPageLayoutSupport.ResolveSpannedContentRowHeightDip(
                intactText,
                SideDetailMaxLines);
            double lossDetailRowHeight = CalculateLossDetailRowHeight(
                data,
                summaryRowHeight,
                borrowRowHeight,
                intactRowHeight,
        ...
        private static double CalculateLossDetailRowHeight(
            ArchiveReturnAbnormalReportPrintData data,
            double summaryRowHeight,
            double borrowRowHeight,
            double intactRowHeight,
            string lossText)
        {
            int approvalCount = data.OutboundApprovalLines?.Count ?? 0;
            // 固定行：借出部门/源单号(2) + 摘要/借出/正常归还 + 灭失情况标签 + 手写 + 借出人签字 + 审批。
            double fixedContentHeight =
                ...
                + ApprovalRowHeight * (1 + approvalCount);
            int fixedRowCount = 3 + 3 + 1 + 1 + approvalCount; // 前2单行+灭失标签 + 摘要/借出/正常 + 手写 + 借出人 + 审批
        ...
        private static Paragraph CreateFooterParagraph()
        {
            ...
            footer.Inlines.Add(new Run("1、资料灭失须由借出人与借出时全部审核审批人（部门审核、资料室签字、生产科签字、分管生产院长签字）手签确认。\n"));
        ...
        private static string BuildFooterNoteText() =>
            "说明：" +
            "1、资料灭失须由借出人与借出时全部审核审批人（部门审核、资料室签字、生产科签字、分管生产院长签字）手签确认。\n" +
            "      2、签字后的灭失情况表应上传系统留存，方可继续打印归还回执并办结入库。";
