...
internal static class NetworkInboundPrintDocumentFactory
{
    ...
    private static Paragraph CreateFooterParagraph(NetworkInboundPrintData data)
    {
        ...
        footer.Inlines.Add(new Run("1、申请提交后，按“线上申请、打印表单、线下审批签字、上传签批单、确认入网交接”的流程办理。\n"));
    ...
    private static string FormatServerPathText(NetworkInboundPrintData data)
    {
        if (string.IsNullOrWhiteSpace(data.ServerPath) || string.Equals(data.ServerPath, "(未指定)", StringComparison.Ordinal))
        {
            return "(未指定)";
        }

        string pathName = data.ServerPath.Trim();
        string physicalPath = data.ServerPhysicalPath?.Trim() ?? string.Empty;
        if (string.IsNullOrWhiteSpace(physicalPath))
        {
            return $"路径名称：{pathName}";
        }
