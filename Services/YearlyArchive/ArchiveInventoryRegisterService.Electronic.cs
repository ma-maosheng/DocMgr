        ...
        DateTime now)
    {
        ...
        foreach (var draft in drafts)
        {
            ...
            items.Add(new YearlyArchiveInventoryRegisterItem
            {
                ...
                ContainerId = candidate.ElectronicArchiveUnitId,
                ContainerCode = candidate.ElectronicArchiveNo,
                BeforeMediaStatus = candidate.BeforeMediaStatus,
                BeforeStorageLocation = candidate.BeforeStorageLocation,
                ProjectName = candidate.ProjectName?.Trim() ?? string.Empty,
                Year = candidate.Year?.Trim() ?? string.Empty,
